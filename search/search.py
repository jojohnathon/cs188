...
def depthFirstSearch(problem: SearchProblem):
    ...
    while(not fringe.isEmpty()):
        ...
        for successor, action, node_cost in problem.getSuccessors(current_state):
...
def aStarSearch(problem: SearchProblem, heuristic=nullHeuristic):
    ...
    fringe = util.PriorityQueue()
    start_node = (problem.getStartState(), [], 0)
    fringe.push(start_node, 0 + heuristic(problem.getStartState(), problem))
    ...
    while(not fringe.isEmpty()):
        ...
        for successor, action, node_cost in problem.getSuccessors(current_state):
            new_actions = actions + [action]
            cost_to_node = current_cost + node_cost
            priority = cost_to_node + heuristic(successor, problem)
            fringe.push((successor, new_actions, cost_to_node), priority)
    return [] #failure
